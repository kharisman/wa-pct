...
export default function Agents({ me }) {
  const [tab, setTab] = useState('agen');
  ...
  const addRole = async (e) => {
    e.preventDefault();
    const name = nr.name.toLowerCase().replace(/[^a-z0-9_]/g, '_'); if (!name) return;
    await post('/roles', { name, label: nr.label || name, perms: [] }); setNr({ name: '', label: '' }); load();
  };
  const delRole = async (name) => { if (confirm('Hapus role ' + name + '?')) { await fetch('/api/roles/' + name, { method: 'DELETE' }); load(); } };

  const TABS = [['agen', '🧑‍💼 Agen'], ['role', '🔑 Role & Akses'], ['master', '🏢 Divisi & Jabatan']];

  return (
    <div className="page">
      <h1 className="page-title">Agen, Role & Divisi</h1>
      <div className="set-tabs">
        {TABS.map(([k, l]) => <button key={k} className={'set-tab' + (tab === k ? ' active' : '')} onClick={() => setTab(k)}>{l}</button>)}
      </div>
            ...
            <tbody>
              {users.map((u) => (
                <tr key={u.email}>
                  ...
                  <td><select className="mini-select" value={u.division || ''} onChange={(e) => updUser(u.email, { division: e.target.value })}>
      ...
      {tab === 'role' && (
        <div className="card nopad">
          ...
          <p className="muted" style={{ padding: '0 14px 14px', fontSize: 12 }}>Centang menu yang boleh diakses. Role <b>admin</b> selalu penuh. Semua role tetap bisa Percakapan/Kontak/Pipeline/Broadcast.</p>
        </div>
      )}
